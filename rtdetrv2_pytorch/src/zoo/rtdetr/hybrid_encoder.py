...
class DCEM(nn.Module):
    """
    Detail Context Enhancement Module + SE Attention
    """
    ...
    def forward(self, x):
        ...
        fused = self.fuse(torch.cat([detail, context], dim=1))
        attn = self.se(fused)
        fused = fused * attn
...
class DGFQ(nn.Module):
    """
    Density-Guided Frequency Query Enhancement
    """
    ...
    def __init__(self, channels):
        ...
        self.density_branch = nn.Sequential(
            ...
            nn.BatchNorm2d(max(channels // 4, 4)),
            nn.SiLU(),
    ...
    def forward(self, x):
        ...
        high_freq = x - smooth
        high_freq = self.freq_branch(high_freq)

        high_freq = high_freq * (1.0 + density)

        out = self.fuse(torch.cat([x, high_freq], dim=1))
...
class BSR(nn.Module):
    """
    Background-Suppressed Recalibration Module

    作用：
    1. foreground_branch 增强小目标前景细节；
    2. background_branch 建模复杂背景干扰；
    3. suppress_gate 生成背景抑制权重；
    4. 减少复杂背景下的误检。
    """
    ...
    def forward(self, x):
        fg = self.foreground_branch(x)
        bg = self.background_branch(x)

        suppress_weight = self.suppress_gate(bg)

        fg = fg * (1.0 + suppress_weight)
        bg = bg * suppress_weight

        fused = self.fuse(torch.cat([x, fg], dim=1))

        out = x + self.alpha * fused - self.beta * bg
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.dcem_blocks = nn.ModuleList([
            DCEM(hidden_dim),
            DCEM(hidden_dim),
            nn.Identity()
        ])

        self.dgfq_blocks = nn.ModuleList([
            DGFQ(hidden_dim),
            DGFQ(hidden_dim),
            nn.Identity()
        ])

        self.bsr_blocks = nn.ModuleList([
            BSR(hidden_dim),
            BSR(hidden_dim),
            nn.Identity()
        ])
    ...
    def forward(self, feats):
        ...
        proj_feats = [
            self.dcem_blocks[i](feat) for i, feat in enumerate(proj_feats)
        ]

        proj_feats = [
            self.dgfq_blocks[i](feat) for i, feat in enumerate(proj_feats)
        ]

        proj_feats = [
            self.bsr_blocks[i](feat) for i, feat in enumerate(proj_feats)
        ]
